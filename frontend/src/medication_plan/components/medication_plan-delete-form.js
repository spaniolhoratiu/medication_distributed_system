...
class MedicationPlanDeleteForm extends React.Component {
    ...
    deleteMedicationPlan(medicationPlan) {
        return API.deleteMedicationPlan(medicationPlan, (result, status, error) => {
            if (result !== null && (status === 200 || status === 201)) {
                console.log("Successfully deleted medicationPlan with id: " + result);
                this.reloadHandler();
            } else {
                this.setState(({
                    errorStatus: status,
        ...
        return (
            <div>
                <FormGroup id='selectedId'>
                    <Label for='selectedIdField'> Selected Medication Plan ID: </Label>
...
export default MedicationPlanDeleteForm;
