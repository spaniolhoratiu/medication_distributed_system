...
class DrugDeleteForm extends React.Component {
    ...
    deleteDrug(drug) {
        return API.deleteDrug(drug, (result, status, error) => {
            if (result !== null && (status === 200 || status === 201)) {
                console.log("Successfully deleted drug with id: " + result);
                this.reloadHandler();
            } else {
                this.setState(({
                    errorStatus: status,
